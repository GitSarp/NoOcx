...
namespace NOocx
{
    ...
    public class IniFile
    {
        ...
        /// <param name="def">默认值</param>
        /// <returns></returns>
        public Boolean ReadBool(string section, string name, Boolean def)
        {
            int tmp;
            if (def)
            {
                tmp = ReadInt(section, name, 1);
            }
            else
            {
                tmp = ReadInt(section, name, 0);
            }
            if (tmp == 0)
                return false;
            else
        ...
        /// <summary>
        /// [扩展]写入Bool数值，如果不存在 节-键，则会自动创建
        /// </summary>
        /// <param name="section">节</param>
        /// <param name="name">键</param>
        /// <param name="Ival">写入值</param>
        public void WriteBool(string section, string name, Boolean Ival)
        {
            if (Ival)
                WriteInt(section, name, 1);
            else
                WriteInt(section, name, 0);
        }
